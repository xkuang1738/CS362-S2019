...
int main()
{
	...
	for (jdx = 0; jdx < TESTCOUNTER; jdx++)
	{
		...
		if (!(testG.handCount[0] == (beforeG.handCount[0] + 2)))
		{
			printf("ERROR with handCount\n");
		}
		if (!(testG.deckCount[0] == (beforeG.deckCount[0] - 3)))
		{
			printf("ERROR with deckCount\n");
		}
		if (!(testG.playedCards[0] == smithy))
		{
			printf("ERROR with played car\n");
		}
		if (!(testG.playedCardCount == 1))
		{
			printf("ERROR with played card count\n");
		}
